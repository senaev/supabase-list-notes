...
import { SupabaseController, SupabaseControllerStatus } from '../controllers/SupabaseController';

const SupabaseControllerStatusContext = createContext<SupabaseControllerStatus | undefined>(
    undefined
);
...
export function SupabaseControllerStatusContextProvider({ children }: PropsWithChildren) {
    ...
    return (
        <SupabaseControllerStatusContext.Provider value={status}>
            {children}
...
export const useSupabaseControllerStatus = (): SupabaseControllerStatus => {
    ...
    if (!status) {
        throw new Error(
            'useSupabaseControllerStatus must be used within a SupabaseClientContextProvider'
        );
    }
